...
class ArgumentParser(object):
    def __init__(self, script_path, tar_file, proxy, extensions, threads, status_code, regex, output_file):
        self.script_path = script_path
        self.parse_config()

        options = self.parse_arguments()
        options.url_list = tar_file
        options.proxy = proxy
        options.extensions = extensions
        options.threads_count = threads
        options.exclude_status_codes = status_code
        options.exclude_regexps = regex
        options.output_file = output_file
        options.use_random_agents = True
